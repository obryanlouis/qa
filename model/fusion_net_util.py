...
def vector_fusion(scope, options, vec_a, vec_b, vec_last_dim_size, vec_apply):
    """Takes two input vectors and applies a fusion to them to get a weighted
       ...
       Output:
            Tensor of shape [batch_size, M, d]
    """
    with tf.variable_scope(scope):
        ...
        a_times_U = tf.nn.relu(multiply_tensors(vec_a, U)) # size = [batch_size, A, k]
        b_times_U = tf.nn.relu(multiply_tensors(vec_b, U)) # size = [batch_size, B, k]
        ...
        return tf.matmul(softmax, vec_apply) # size = [batch_size, A, d]
