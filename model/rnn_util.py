...
def _get_or_create_attention_variables(options, attention_input, input_dim,
    ...
    with tf.variable_scope(scope):
        ...
        b = tf.get_variable("b", shape=[1, options.rnn_size])
...
def run_attention(sq_dataset, options, inputs, input_dim, attention_input, attention_dim,
    ...
    with tf.variable_scope(scope):
        ...
        Hr_forward  = tf.stack(forward_outputs , axis=2) # size = [batch_size, rnn_size, max_ctx_length]
        Hr_backward = tf.stack(backward_outputs, axis=2) # size = [batch_size, rnn_size, max_ctx_length]
        Hr = tf.concat([Hr_forward, Hr_backward], axis=1) # size = [batch_size, 2 * rnn_size, max_ctx_length]
        Hr = tf.transpose(Hr, perm=[0, 2, 1])
        return Hr

def _build_match_lstm(options, batch_size, lstm_cell,
        input_index, Wp, Wr,
        WqHq, w, b, attention_inputs, inputs, state, outputs,
    ...
    alpha = tf.nn.softmax(wG) # size = [batch_size, attention_dim]
    Yalpha = tf.reshape(
